...
function Deps(opts) {
    ...
    if (typeof opts.includeSource !== 'undefined') {
        if (typeof opts.includeSource === 'string') {
            this.includeSource = opts.includeSource === 'true';
        } else {
...
Deps.prototype.readFile = function(file, id, pkg) {
    var self = this;
    var tr = through();
    if (this.cache && this.cache[file] && self.includeSource) {
...
Deps.prototype.walk = function(id, parent, cb) {
    ...
    self.resolve(id, parent, function(err, file, pkg) {
        ...
        if (err && rec.source && self.includeSource) {
            file = rec.file;
        ...
        if (rec.source && self.includeSource) {
            var ts = self.getTransforms(file, pkg);
            ts.pipe(concat(function(body) {
                rec.source = body.toString('utf8');
                fromSource(rec.source);
    ...
    function fromDeps(file, src, pkg, deps) {
        ...
        function done() {
            ...
            if (!rec.source && self.includeSource) rec.source = src;
